...
def compute_content_cost(a_C, a_G):
    ...
    constant = 1 / (n_H * n_W * n_C * 4)
    J_content = constant * tf.reduce_sum(tf.square(tf.subtract(a_C_unrolled, a_G_unrolled)))
    ### END CODE HERE ###
...
def compute_layer_style_cost(a_S, a_G):
    ...
    constant =(4 * n_C* n_C * (n_H * n_W)* (n_H * n_W))
    J_style_layer = tf.reduce_sum(tf.square(tf.subtract(GS, GG)))/ constant
